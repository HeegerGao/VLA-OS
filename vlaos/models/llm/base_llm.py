...
class HFCausalLLMBackbone(LLMBackbone, ABC):
    def __init__(
        ...
        if not pretrained:  # this is for VLA training to avoid loading weights from HF because we provide the trained weights by ours
        ...
        if not pretrained:
            self.tokenizer = AutoTokenizer.from_pretrained(
                hf_hub_path,   # ← 本地目录，里面要有 vocab.json, tokenizer_config.json, tokenizer.json, config.json 等文件
