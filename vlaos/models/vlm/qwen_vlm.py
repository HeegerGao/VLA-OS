"""
PyTorch Module defining a VLM, our general interface for defining the various different VLMs in our work.
...
from vlaos.models.llm import LLMBackbone
from vlaos.models.llm.prompting import PromptBuilder
from vlaos.models.vision import VisionBackbone
from vlaos.models.vlm import VLM
from vlaos.models import FusedMLPProjector, LinearProjector, MLPProjector
...
class QwenVLM(VLM):
    def __init__(
        ...
        self._supports_cache_class = Cache
    ...
    def from_pretrained(
        ...
        """Initialize a QwenVLM from a pretrained checkpoint, freezing all weights, tailored for inference."""
        vlm = cls(
            model_id,
            vision_backbone,
            llm_backbone,
        ...
        ), "QwenVLM `from_pretrained` expects checkpoint with keys for `projector` AND `llm_backbone`!"
    ...
    def load_from_checkpoint(self, stage: str, run_dir: Path, pretrained_checkpoint: Optional[Path] = None) -> None:
        ...
        assert stage in {"align", "finetune", "full-finetune", "freeze"}, f"Stage {stage} is not supported!"

        # If we're running a `no-align` architecture, we're good!
        if self.arch_specifier.startswith("no-align"):
            print(
                f"QwenVLM with `{self.arch_specifier = }` does not require pretrained weights!", ctx_level=1
            )
            return
        ...
        overwatch.info("Stage `finetune` or `freeze` requires `align` pretrained weights", ctx_level=1)
    ...
    def get_fsdp_wrapping_policy(self) -> Callable:
        ...
        # Get module wrapping policy around `self.projector`
