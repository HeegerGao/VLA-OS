...
from vlaos.models.vision.base_vision import ImageTransform, LetterboxPad, VisionBackbone, unpack_tuple, compute_sequence_patches
...
class DinoSigLIPViTBackbone(VisionBackbone):
    def __init__(self, vision_backbone_id: str, image_resize_strategy: str, default_image_size: int = 224, image_sequence_len = 1, pretrained=True) -> None:
    ...
    def forward(self, pixel_values: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if self.image_sequence_len == 1:
            ...
            dino_patches = compute_sequence_patches(pixel_values['dino'], self.dino_featurizer, self.image_sequence_len)
            siglip_patches = compute_sequence_patches(pixel_values['siglip'], self.siglip_featurizer, self.image_sequence_len)
        return torch.cat([dino_patches, siglip_patches], dim=2) # [B, T*patch_num, patch_dim+patch_dim]
    ...
    def num_patches(self) -> int:
        ...
        return self.dino_featurizer.patch_embed.num_patches # TODO: Check if we need to * self.image_sequence_len
