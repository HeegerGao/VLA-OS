...
from vlaos.datasets.cot_utils import get_cot_database_keys, get_cot_tags_list
from vlaos.datasets.rlds import obs_transforms, traj_transforms
from vlaos.datasets.rlds.utils import goal_relabeling, task_augmentation
from vlaos.datasets.rlds.utils.data_utils import (
    NormalizationType,
    allocate_threads,
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    load_reasoning: bool = True,
    reasoning_dataset_path: str = "reasoning.json",    # this file is assumed in the same directory as the tfrecord files
    goal_image_step: int = 0,
    ...
    reasoning_dataset_file = os.path.join(data_dir, name, reasoning_dataset_path)
    if load_reasoning:
        if os.path.isfile(reasoning_dataset_file):
            print(f"Loading from local checkpoint path `{reasoning_dataset_file}`.")
        else:
            raise FileNotFoundError(f"Dataset file `{reasoning_dataset_file}` not found!")

        with open(reasoning_dataset_file, "r") as f:
    ...
    def make_tf_dict(raw_dict):
        ...
        def reasoning_dict_to_str(d):
            ...
            return "@".join(f"{tag}@{part}" for tag, part in reasoning_parts if part != "")
        ...
        for file_name in raw_dict.keys():
            for episode_id in raw_dict[file_name].keys():
                ...
                for i in raw_dict[file_name][episode_id]["reasoning"].keys():
                    if "dexart" in reasoning_dataset_file:
                        if "toilet" in reasoning_dataset_file:
                            key_file_name = "toilet_" + file_name
                        elif "laptop" in reasoning_dataset_file:
                            key_file_name = "laptop_" + file_name
                        elif "faucet" in reasoning_dataset_file:
                            key_file_name = "faucet_" + file_name
                        elif "bucket" in reasoning_dataset_file:
                            key_file_name = "bucket_" + file_name
                        else:
                            raise ValueError(
                                f"Unknown dataset file name {reasoning_dataset_file}, "
                                "expected to contain 'toilet', 'laptop', 'faucet' or 'bucket'."
                            )
                    elif "deformable" in reasoning_dataset_file:
                        if "rope" in reasoning_dataset_file:
                            key_file_name = "rope_" + file_name
                        elif "fold" in reasoning_dataset_file:
                            key_file_name = "fold_" + file_name
                        elif "jean" in reasoning_dataset_file:
                            key_file_name = "jean_" + file_name
                        else:
                            raise ValueError(
                                f"Unknown dataset file name {reasoning_dataset_file}, "
                                "expected to contain 'rope', 'fold', or 'jean'."
                            )
                    elif "peract" in reasoning_dataset_file:
                        if "handover" in reasoning_dataset_file:
                            key_file_name = "handover_" + tf.strings.split(file_name, sep="/")[-1]
                        elif "rope" in reasoning_dataset_file:
                            key_file_name = "rope_" + tf.strings.split(file_name, sep="/")[-1]
                        elif "sweep" in reasoning_dataset_file:
                            key_file_name = "sweep_" + tf.strings.split(file_name, sep="/")[-1]
                        elif "lift" in reasoning_dataset_file:
                            key_file_name = "lift_" + tf.strings.split(file_name, sep="/")[-1]
                        else:
                            raise ValueError(
                                f"Unknown dataset file name {reasoning_dataset_file}, "
                                "expected to contain 'handover', 'rope', 'sweep' or 'lift'."
                            )                        
                    else:
                        key_file_name = file_name
                    keys.append(key_file_name + "_" + str(episode_id) + "_" + i)
                    reasoning_dict = raw_dict[file_name][episode_id]["reasoning"][i]

                    gripper_lookahead_n = 16  # list this many future positions of the gripper
                            ...
                            if 0 <= int(i) < len(trajectory_features["gripper_position"]):
                                ...
                                for j in range(gripper_lookahead_n):
                                    if int(i) + j < len(trajectory_features["gripper_position"]):
                                        if trajectory_features["gripper_position"][int(i) + j] is not None:
                                            future_positions.append(trajectory_features["gripper_position"][int(i) + j])
                                        else:
                                            future_positions.append([0, 0]) # for deformable manipulation datasets, where there could be some None in the beginning
                                    else:
                                        future_positions += future_positions[-2:]   # this means padding with the last frame gripper action
                            ...
                            if 0 <= int(i) < len(trajectory_features["bboxes"]):
                                if len(trajectory_features["bboxes"][int(i)]) > 0:
                                    ...
                                    reasoning_dict["bboxes"] = ", ".join(
                                        [f"{name} {box!s}" for name, box in boxes_list]
                                    )

                           
                    # visual planning part
                    # note visual_bboxes and visual_bbox are different
                    reasoning_dict["visual_bboxes"] = ""
                    if "visual_bbox" in trajectory_features.keys():
                        if trajectory_features["visual_bbox"] is not None:
                            if 0 <= int(i) < len(trajectory_features["visual_bbox"]):
                                if len(trajectory_features["visual_bbox"][int(i)]) > 0:
                                    boxes_list = trajectory_features["visual_bbox"][int(i)]
                                    reasoning_dict["visual_bboxes"] = ", ".join(
                                        [f"{name} {box!s}" for name, box in boxes_list]
                                    )
                            
                    reasoning_dict["visual_flow"] = ""
                    if "visual_flow" in trajectory_features.keys():
                        if trajectory_features["visual_flow"] is not None:
                            if 0 <= int(i) < len(trajectory_features["visual_flow"]):
                                future_positions = []
                                for j in range(gripper_lookahead_n):
                                    if int(i) + j < len(trajectory_features["visual_flow"]):
                                        if trajectory_features["visual_flow"][int(i) + j] is not None:
                                            future_positions.append(trajectory_features["visual_flow"][int(i) + j])
                                        else:
                                            future_positions.append([0, 0])
                                    else:
                                        future_positions.append(future_positions[-1])   # this means padding with the last frame gripper action

                                reasoning_dict["visual_flow"] = str(future_positions)

                    reasoning_dict["visual_affordance"] = ""
                    if "visual_affordance" in trajectory_features.keys():
                        if trajectory_features["visual_affordance"] is not None:
                            if 0 <= int(i) < len(trajectory_features["visual_affordance"]):
                                reasoning_dict["visual_affordance"] = str(trajectory_features["visual_affordance"][int(i)])
    ...
    def restructure(traj):
        ...
        for new, old in depth_obs_keys.items():
            ...
            # transfer to float32
            new_obs[f"depth_{new}"] = tf.cast(new_obs[f"depth_{new}"], tf.float32)
        ...
        if state_obs_keys:
            ...
            extrinsics_keys = [
                key for key in state_obs_keys
                if key is not None and "extrinsics" in key
            ]
            if len(extrinsics_keys) > 0:
                camera_extrinsics = {}
                for key in extrinsics_keys:
                    camera_extrinsics[key] = tf.cast(old_obs[key], tf.float32)
            
            # for camera intrinsics
            intrinsics_keys = [
                key for key in state_obs_keys
                if key is not None and "intrinsics" in key
            ]
            if len(intrinsics_keys) > 0:
                camera_intrinsics = {}
                for key in intrinsics_keys:
                    camera_intrinsics[key] = tf.cast(old_obs[key], tf.float32)            
        ...
        if "dexart" in reasoning_dataset_file:
            basename = tf.strings.split(file_name, sep="/")[-1]
            stem = tf.strings.split(basename, sep=".")[0]
            episode_id = tf.strings.split(stem, sep="_")[1]
            if "toilet" in reasoning_dataset_file:
                file_name = "toilet"
            elif "laptop" in reasoning_dataset_file:
                file_name = "laptop"
                ...
                file_name = "bucket"
            file_name = file_name + "_" + episode_id
        elif "deformable" in reasoning_dataset_file:
            ...
            episode_id = tf.strings.split(file_name, sep="/")[-1]
            if "handover" in reasoning_dataset_file:
                file_name = "handover"
            elif "rope" in reasoning_dataset_file:
                file_name = "rope"
        ...
        if load_reasoning:
            if "episode_id" not in traj["traj_metadata"]["episode_metadata"]:
                episode_id = "0"
            else:
                episode_id = traj["traj_metadata"]["episode_metadata"]["episode_id"][0]
            if "deformable" in reasoning_dataset_file or "peract" in reasoning_dataset_file:
                episode_id = "0"
        ...
        if len(extrinsics_keys) > 0:
            for key in extrinsics_keys:
                traj[key] = camera_extrinsics[key]

        if len(intrinsics_keys) > 0:
            for key in intrinsics_keys:
                traj[key] = camera_intrinsics[key]
    ...
    if sample_fraction < 1.0:
        assert sample_fraction > 0.0, "sample_fraction should be in (0, 1)"
        print(f"Sampling dataset by language instruction with fraction {sample_fraction} ..")
        dataset = sample_exact_by_instruction(dataset, sample_fraction, seed=42)

        # =========== recalculate the statistics =========== #
        print("Recomputing dataset statistics on the downsampled dataset …")
        dataset_statistics = get_dataset_statistics(
            dataset,
            hash_dependencies=(
                str(builder.info) + f"_sample{sample_fraction}",
                str(state_obs_keys),
                inspect.getsource(standardize_fn) if standardize_fn is not None else "",
            ),
            save_dir=builder.data_dir,
...
def apply_frame_transforms(
    ...
    *,
    ...
    def apply_obs_transform(fn: Callable[[Dict], Dict], frame: Dict, goal_image=True) -> Dict:
        ...
        if goal_image:
            frame["goal_image"] = fn(frame["goal_image"], resize_size=(256, 256))   # we assume goal images are 256x256
        return frame
...
def make_interleaved_dataset(
    ...
    *,
    ...
    for dataset_kwargs in dataset_kwargs_list:
        ...
        _, dataset_statistics = make_dataset_from_rlds(**data_kwargs, train=train, load_reasoning=load_reasoning, sample_fraction=sample_fraction)
