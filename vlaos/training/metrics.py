...
class VLAMetrics:
    def __init__(
        ...
        self.state = {
            "loss_raw": deque(maxlen=grad_accumulation_steps),
            "loss": deque(maxlen=window_size),
            "language_planning_loss": deque(maxlen=window_size),
            "visual_planning_loss": deque(maxlen=window_size),
            "image_foresight_planning_loss": deque(maxlen=window_size),
    ...
    def push(self) -> str:
        # Note :: Raw Loss is an Average over Gradient Accumulation Steps --> No Smoothing!
        language_planning_loss = torch.stack(list(self.state["language_planning_loss"])).mean().item()
        visual_planning_loss = torch.stack(list(self.state["visual_planning_loss"])).mean().item()
        image_foresight_planning_loss = torch.stack(list(self.state["image_foresight_planning_loss"])).mean().item()
        ...
        for ds, tracker in self.dataset_trackers.items():
            dataset_metrics.update(
                {
                    f"{ds}/Language Planning Loss": torch.stack(list(tracker.state["language_planning_loss"])).mean().item(),
                    f"{ds}/Visual Planning Loss": torch.stack(list(tracker.state["visual_planning_loss"])).mean().item(),
                    f"{ds}/Image Foresight Planning Loss": torch.stack(list(tracker.state["image_foresight_planning_loss"])).mean().item(),
        ...
        self.log(
            ...
            metrics={
                ...
                f"{prefix}/Language Planning Loss": language_planning_loss,
                f"{prefix}/Visual Planning Loss": visual_planning_loss,
                f"{prefix}/Image Foresight Planning Loss": image_foresight_planning_loss,
